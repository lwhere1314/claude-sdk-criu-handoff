...
RUNTIME_DIR=${RUNTIME_DIR:-"/tmp/claude-sdk-criu-handoff-${SUDO_UID:-$UID}"}
ARTIFACT_ROOT=${ARTIFACT_ROOT:-"$ROOT_DIR/artifacts"}
IMAGE=${CANARY_IMAGE:-"claude-sdk-criu-handoff:local"}
CONTAINER=claude-sdk-criu-handoff
...
if (( EUID != 0 )); then
  exec sudo \
    --preserve-env=ENV_FILE,RUNTIME_DIR,ARTIFACT_ROOT,CANARY_IMAGE,CODING_PLAN_BASE_URL,CODING_PLAN_API_KEY,CANARY_MODEL,CANARY_NONCE \
    "$0" "$@"
fi
...
if [[ -f "$ENV_FILE" ]]; then
  ...
  echo "Missing $ENV_FILE and exported Coding Plan credentials." >&2
  echo "Copy .env.example to .env or export CODING_PLAN_BASE_URL and CODING_PLAN_API_KEY." >&2
  exit 2
fi

: "${CODING_PLAN_BASE_URL:?Set CODING_PLAN_BASE_URL in .env}"
: "${CODING_PLAN_API_KEY:?Set CODING_PLAN_API_KEY in .env}"
CANARY_MODEL=${CANARY_MODEL:-kimi-k2.6}
CANARY_NONCE=${CANARY_NONCE:-"CRIU-$(date +%s)-$RANDOM"}

case "$CODING_PLAN_BASE_URL$CODING_PLAN_API_KEY" in
...
dockerd \
  ...
  --containerd-namespace claude-sdk-criu-handoff \
  --containerd-plugins-namespace claude-sdk-criu-handoff-plugins \
...
umask 077
{
  ...
  printf 'CLAUDE_CONFIG_DIR=/workspace/.claude\n'
  printf 'IS_SANDBOX=1\n'
} > "$ENV_SNAPSHOT"
...
echo "Waiting for the first native SDK turn..."
wait_for_marker NATIVE_CANARY_READY 900 || {
...
echo "Waiting for the resumed native SDK turn..."
wait_for_marker NATIVE_CANARY_COMPLETE 900 || {
...
docker -H "$DOCKER_HOST_VALUE" logs "$CONTAINER" > "$ARTIFACT_DIR/container.log" 2>&1
docker -H "$DOCKER_HOST_VALUE" cp \
  "$CONTAINER:/workspace/native-resume-evidence.txt" \
  "$ARTIFACT_DIR/evidence.txt"

grep -q 'same_session=True evidence_ok=True' "$ARTIFACT_DIR/container.log"
grep -Fxq "NATIVE_SESSION_RESUMED $CANARY_NONCE" "$ARTIFACT_DIR/evidence.txt"

SESSION_ID=$(sed -n \
  's/.*NATIVE_CANARY_COMPLETE original_session=\([^ ]*\).*/\1/p' \
  "$ARTIFACT_DIR/container.log" | tail -n 1)
cat > "$ARTIFACT_DIR/result.md" <<EOF
# Native Claude SDK + CRIU canary

- Result: PASS
- Model: \`$CANARY_MODEL\`
- Native session ID: \`$SESSION_ID\`
- Host PID before checkpoint: \`$BEFORE_PID\`
- Host PID after restore: \`$AFTER_PID\`
- Checkpoint state: \`$CHECKPOINT_STATE\`
- Same native session: \`true\`
- Evidence validated: \`true\`

No API key or container inspection output is retained in this directory.
EOF

chown -R "$OWNER_UID:$OWNER_GID" "$ARTIFACT_DIR"
echo "PASS: native SDK session resumed after CRIU restore"
